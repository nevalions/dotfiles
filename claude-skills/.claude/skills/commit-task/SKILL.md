...
6. Make the bd change leave this machine (a closure after the last commit is
   ...
   - repos still on JSONL export (a git-tracked `.beads/issues.jsonl`, written
     by the pre-commit hook): the hook is the export path, so close the issue
     BEFORE the final commit, or make a follow-up commit for the export.
